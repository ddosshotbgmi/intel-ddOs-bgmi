...
TOKEN = '7890504030:AAFWLZEVynWnQbZ7hlUZSsPYpRbYHuxZFTg'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -1002448433920
CHANNEL_ID = -1002448433920
error_channel_id = -1002448433920
...
def approve_or_disapprove_user(message):
    ...
    # Remove below if not adimin string: to get admin access: Tg bot-ESiOS2🧡
    if not is_admin:
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @KaliaYtOwner.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @KaliaYtOwner* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @KaliaYtOwner* 🙏"  # Contact owner message
    else:
...
def start_message(message):
    try:
        bot.send_message(message.chat.id, "*🌍 WELCOME TO KALIAYT DDOS WORLD!* 🎉\n\n"
